...
function colorMarker(gdp_pc){
  let color = "";

  if (gdp_pc > 100000) {
    color = "yellow";
  }
  else if (gdp_pc > 75000) {
    color = "blue";
  }
  else if (gdp_pc > 50000) {
    color = "green";
  }
  else {
    color = "violet";
...
function markerSize(gdp_pc) {
  return Math.sqrt(gdp_pc) * 500;
}

// Loop through the countries array.
countries.forEach(function(country){
  console.log(country.gdp_pc,colorMarker(country.gdp_pc), markerSize(country.gdp_pc));
  
  L.circle(country.location, {
    color: colorMarker(country.gdp_pc),
      fillColor: colorMarker(country.gdp_pc),
      radius: markerSize(country.gdp_pc),
      fillOpacity: 0.5
  }).bindPopup(`<h1>${country.name}</h1> <hr> <h3>GDP Per Capita (USD): ${country.gdp_pc}</h3>`).addTo(myMap)
});
